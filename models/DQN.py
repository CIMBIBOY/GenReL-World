...
class PPO:
    ...
    def update_critic(self, states, actions, rewards, dones, next_states):
        ...
        with torch.set_grad_enabled(True):
            # Preprocess the states and next_states
            states = self.preprocess_states(states)
            next_states = self.preprocess_states(next_states)
    ...
    def preprocess_states(self, states):
        """Preprocess the input states by normalizing the features."""
        # Compute the mean and standard deviation of the state features
        state_mean = np.mean([s for s in states], axis=0)
        state_std = np.std([s for s in states], axis=0)

        # Normalize the state features
        preprocessed_states = [(s - state_mean) / (state_std + 1e-8) for s in states]

        # Convert the preprocessed states to PyTorch tensors
        preprocessed_states_tensor = torch.stack([torch.from_numpy(np.array(s)) for s in preprocessed_states], dim=0).float().unsqueeze(1)

        return preprocessed_states_tensor
                ...
                if total_steps < 100000:
                    ...
                    if np.random.rand() < self.epsilon:
                        ...
                        if not first_ppo:
                            ...
                            first_ppo = True
                        states_tensor = self.preprocess_states(allstates[-32:])
