...
class SawyerPickPlaceEnvV2(SawyerXYZEnv):
    ...
    '''
    Rewarding the agent for getting closer to the object (_gripper_caging_reward() function).
    Rewarding the agent for successfully grasping the object (_gripper_caging_reward() function).
    Rewarding the agent for manipulating the object in the desired way (compute_reward() function).
    The _gripper_caging_reward() function computes a reward based on the distance between the object and the gripper pads, 
        as well as the distance between the object and the target position in the x-z plane. 
    The compute_reward() function then combines this "caging" reward with a reward for the object being "in place" (i.e., close to the target position).
    
    Additionally, the compute_reward() function provides an extra reward if the object is lifted above the initial position and the gripper is open.
    '''
    ...
    def own_compute_reward(self, action, obs):
        _TARGET_RADIUS = 0.05
        tcp = self.tcp_center
        obj = obs[4:7]
        tcp_opened = obs[3]
